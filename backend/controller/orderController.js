...
export const getMyOrder = async (req, res) => {
  try {
    ...
    console.error("Error fetchinguser Orders: ", error);
...
export const getUserOrder = async (req, res) => {
  try {
    ...
    console.log("Error while fetching your order", error);
...
export const getSalesData = async (req, res) => {
  try {
    ...
    const thirtDayAgo = new Date();
thirtDayAgo.setDate(thirtDayAgo.getDate() - 30);
    const salesByDate = await Order.aggregate([
      { $match: { status: "Paid", createdAt: { $gte: thirtDayAgo } } },
      {
        $group: {
          _id: {
